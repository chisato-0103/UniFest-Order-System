...
function OrderPage() {
  ...
  const calculateItemPrice = (product: SimpleProduct, toppings: SimpleTopping[]) => {
    return product.price + toppings.reduce((sum, topping) => sum + topping.price, 0);
  };
  ...
  // 営業状況に応じた表示制御
  const isOrderingAvailable = systemState.営業状況 === "営業中" && !systemState.緊急停止状態;
        ...
        {!isOrderingAvailable && (
          <Alert severity="warning" sx={{ mb: 2 }}>
            ...
            <Typography variant="body2">
              {systemState.営業状況 === "準備中" && "営業準備中です。しばらくお待ちください。"}
              {systemState.営業状況 === "終了" && "本日の営業は終了いたしました。"}
        ...
        {dummyProducts.map((product) => (
          <Card 
            ...
            sx={{ 
              ...
              cursor: product.available && isOrderingAvailable ? "pointer" : "default",
              "&:hover": product.available && isOrderingAvailable ? { 
                boxShadow: 4,
                transform: "translateY(-2px)",
                transition: "all 0.2s ease-in-out"
              } : {}
            }}
            onClick={() => product.available && isOrderingAvailable && handleProductClick(product)}
          >
            <CardContent>
        ...
        <DialogContent>
          {selectedProduct && (
            <Box>
              ...
              <FormGroup>
                {dummyToppings.filter(t => t.available).map((topping) => (
                  <FormControlLabel
                    key={topping.id}
                    control={
                      <Checkbox
                    ...
                    label={
                      <Box sx={{ display: "flex", justifyContent: "space-between", width: "100%" }}>
                        ...
                        <span>{topping.price > 0 ? `+¥${topping.price}` : "無料"}</span>
                      </Box>
                    }
                  />
                ))}
              ...
              <Typography variant="h6" color="primary">
                合計: ¥{calculateItemPrice(selectedProduct, selectedToppings).toLocaleString()}
              </Typography>
            </Box>
          )}
        </DialogContent>
                      ...
                      <Box>
                        {item.selectedToppings.length > 0 && (
                          <Typography variant="body2" color="text.secondary">
                            トッピング: {item.selectedToppings.map(t => t.name).join(", ")}
                          </Typography>
                        )}
