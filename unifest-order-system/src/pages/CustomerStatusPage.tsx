...
function CustomerStatusPage() {
  ...
  const getEstimatedWaitTime = () => {
    ...
    const diffMinutes = Math.max(0, Math.ceil((pickupTime.getTime() - now.getTime()) / (1000 * 60)));
    return diffMinutes;
  };
                  ...
                  <Step key={step.label}>
                      ...
                      StepIconComponent={({ active, completed }) => (
                        ...
                        >
                          {completed ? <CheckCircleIcon /> : 
                           step.label === '注文受付' ? <CartIcon /> :
                           step.label === '調理中' ? <RestaurantIcon /> :
                           step.label === '調理完了' ? <ScheduleIcon /> :
                           <QrCodeIcon />}
                        </Avatar>
                      )}
                    >
                    ...
                    <StepContent>
                      <Typography variant="body2" color="text.secondary">
                        ...
                        {step.label === '調理中' && '美味しいたこ焼きを調理中です'}
                        {step.label === '調理完了' && '調理が完了しました'}
                        {step.label === '受け取り' && 'カウンターでお受け取りください'}
                      </Typography>
                    </StepContent>
                  </Step>
                ))}
