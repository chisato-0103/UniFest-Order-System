...
<!-- このファイルは、プロジェクトの説明書です。小学生でもわかるように書いてあります！ -->
...
<!-- 簡単に言うと：大学祭でたこ焼き屋さんを開く時に、お客さんの注文を管理するアプリです！ -->

## ✨ 主な機能

### 🏪 お店で使える基本機能

- 📱 **注文管理**: スマートフォン対応の注文受付システム（お客さんがスマホで注文できる）
- 👨‍🍳 **厨房管理**: 調理状況のリアルタイム管理（キッチンでどの注文を作っているかわかる）
- 🔔 **通知システム**: 音声・視覚通知による効率的な連絡（「ピンポン」って音で知らせてくれる）
- 💳 **決済・受け渡し**: 支払い処理と受け渡し管理（お金をもらって商品を渡す画面）
- 📊 **売上管理**: リアルタイム売上集計・履歴管理（今日いくら売れたかすぐわかる）
...
- [x] **バックエンド API 統合** (Node.js + Express + PostgreSQL)
- [x] **リアルタイム機能** (Socket.io・自動更新)
- [x] **QR コード機能** (注文完了時生成・顧客ページ連携)
- [x] **音声通知システム** (設定・テスト・実行機能)
- [x] **在庫管理機能** (自動計算・アラート・履歴)
- [x] **待ち時間表示機能** (動的計算・混雑度表示)
- [x] **たこ焼き器管理** (稼働状況・温度管理・メンテナンス)
- [x] **本番ビルド対応** (環境変数・最適化・エラーハンドリング)
- [x] **デプロイ準備完了** (Render 対応・設定ファイル・ドキュメント)
