...
function PaymentPage() {
  ...
  const stats = {
    ...
    unpaidAmount: orders.filter(o => o.payment_status === '未払い').reduce((sum, o) => sum + o.total_amount, 0),
    readyForPayment: orders.filter(o => o.status === '調理完了' && o.payment_status === '未払い').length,
  };
      ...
      setOrders(prev => 
        prev.map(order => 
          order.order_id === selectedOrder.order_id 
            ? { ...order, payment_status: '支払済み', updated_at: new Date().toISOString() }
            : order
        )
      );
      ...
      <Card>
        <CardContent>
          ...
          {filteredOrders.length === 0 ? (
            <Alert severity="info">
              {searchQuery ? '検索条件に該当する注文がありません' : '支払い待ちの注文はありません'}
            </Alert>
          ) : (
                ...
                <TableBody>
                  {filteredOrders.map((order) => (
                    <TableRow 
                      key={order.order_id}
                      sx={{ 
                        bgcolor: order.status === '調理完了' && order.payment_status === '未払い' 
                    ...
                    >
                      <TableCell>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          {order.status === '調理完了' && order.payment_status === '未払い' && (
                      ...
                      <TableCell>
                        <Chip
                          label={order.status}
                          color={getStatusColor(order) as 'success' | 'info' | 'warning' | 'default'}
                          size="small"
                        />
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <Typography 
                          variant="body2" 
                          color={getElapsedTime(order.created_at) > 20 ? 'error' : 'text.secondary'}
                        >
                        ...
                        <Box sx={{ display: 'flex', gap: 1 }}>
                          {order.payment_status === '未払い' && (
                            <Button
                              ...
                              color={order.status === '調理完了' ? 'success' : 'primary'}
                            >
                              {order.status === '調理完了' ? '支払い処理' : '調理待ち'}
                            </Button>
                          )}
      ...
      <Dialog open={paymentDialog} onClose={() => setPaymentDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          {selectedOrder && (
            <Box>
              ...
              {receivedAmount && parseFloat(receivedAmount) >= selectedOrder.total_amount && (
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={!receivedAmount || !selectedOrder || parseFloat(receivedAmount) < selectedOrder.total_amount}
